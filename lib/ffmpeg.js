...
module.exports = function(req, res) {
	...
	var cacheKey = util.format('%s-%s-%d.jpg', req.id, time, width);
	...
	fs.exists(cachePath, function(exists) {
		...
		var command = util.format('ffmpeg -ss %s -i %s -frames:v 1 -filter:v scale=%d:-1 -f image2 pipe:1', time, req.video.path, width);
